...
for((i=1;i<=$cycle_total;i++));do
    ...
    energies=$(grep -m $(expr $i - 1) 'TOTAL ENERGY(DFT)(AU)' ./$output_file | awk '{print $(NF-3)}')
    ...
    if [ -z $var_type ] ; then
        ...
        energies=$(grep -m $(expr $i - 1) 'TOTAL ENERGY(DFT)(AU)' ./$output_file | awk '{print $(NF-4)}')
    ...
    printf "\n" >> $plot_file
    if [ -n "$energies" ] ; then
        for ii in $energies ; do
            if [ `expr ${total_energy} \> ${ii}` -eq 0 ] ; then
                printf "%s" xxx  >> $plot_file
            break
            fi
        done
    fi
    printf "\t %d \t %15.10f \t %d" $cycle_number $total_energy $SCF_CYCLES >> $plot_file
