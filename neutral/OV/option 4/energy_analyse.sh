...
plot_file=$PWD/${input_file%.*}__data.ssv
...
echo -n "# CYCLE NUMBER | TOTAL ENERGY (Ha) | SCF_CYCLES | MAX_GRADIENT(0.000450) | RMS_GRADIENT(0.000300) | MAX_DISPLAC.(0.001800) | RMS_DISPLAC.(0.001200) | TRUST RADIUS" >> $plot_file
...
for((i=1;i<=$cycle_total;i++));do
    ...
    read TRUST_RADIUS <<<  $(sed -n "${line_number1},${line_number2}p" $output_file | grep -e 'UPDATED TRUST RADIUS' | awk '{print $4}')
